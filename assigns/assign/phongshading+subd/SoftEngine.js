...
(function (SoftEngine) 
{
    ...
    var Device = (function () 
    {
        ...
        Device.prototype.processScanLine = function (data, va, vb, vc, vd, color) 
        {
            ...
            // draw normals
            //this.drawBline(va.Normal, vb.Normal);

            // end draw normals
            var normalA = new BABYLON.Vector3(
            ...
            var normalB = new BABYLON.Vector3(
            ...
            for (var x = sx; x < ex; x++) 
            {
                ...
                var z = this.interpolate(z1, z2, t);
                
                var pixelNormal = new BABYLON.Vector3(
                    this.interpolate(normalA.x, normalB.x, t),
                    this.interpolate(normalA.y, normalB.y, t),
                    this.interpolate(normalA.z, normalB.z, t)
                    );
                var ndotl = this.computeNDotL(va.WorldCoordinates, pixelNormal, new BABYLON.Vector3(5, 5, 10));

                // the colors here are what should change based on the light 
                // interpolate the color between to the two lines for normals

                //   /\
                //  /  \
                // /____\
                //ndotl = this.computeNDotL(new BABYLON.Vector3(x, data.currentY,z), BABYLON.Vector3.Cross(x,z), new BABYLON.Vector3(0,10,10));
                //var norm = BABYLON.Vector3.Normalize(ndotl);
               
                this.drawPoint(new BABYLON.Vector3(x, data.currentY, z), 
                    new BABYLON.Color4(color.r * ndotl, color.g * ndotl, color.b * ndotl, 1));
                //this.drawPoint(new BABYLON.Vector3(x, data.currentY, z), new BABYLON.Color4(1, 1, 1, 1));
            }
        };
        ...
        Device.prototype.CreateMeshesFromJSON = function (jsonObject) 
        {
            ...
            for (var meshIndex = 0; meshIndex < jsonObject.meshes.length; meshIndex++) 
            {
                ...
                // console.log("VS" + verticesStep);
                var verticesCount = verticesArray.length / verticesStep;
                console.log(verticesCount + "  ===  " + verticesArray.length);
